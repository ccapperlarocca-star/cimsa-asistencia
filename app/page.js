...
const Logo = ({ size = 28 }) => (
  <svg width={size * 3.2} height={size} viewBox="0 0 96 30" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="40" y="22" fontFamily="sans-serif" fontWeight="700" fontSize="13" fill={BRAND.teal}>SOLUCIONES</text>
  </svg>
);
...
const TopBar = ({ user, onLogout }) => (
  <div style={{ background: BRAND.navy, padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <i className="ti ti-clock" style={{ fontSize: 20, color: BRAND.teal }} aria-hidden="true"></i>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <button onClick={onLogout} style={{ fontSize: 12, padding: "4px 12px", background: "transparent", border: `0.5px solid rgba(255,255,255,0.3)`, color: "#fff", borderRadius: 6, cursor: "pointer" }}>Salir</button>
    </div>
  </div>
);
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: BRAND.navyLight }}>
      <div style={{ background: "#fff", border: `0.5px solid #dde3f0`, borderRadius: 16, padding: "2.5rem 2rem", width: 340, boxSizing: "border-box" }}>
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: BRAND.navy, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem" }}>
            <i className="ti ti-clock" style={{ fontSize: 30, color: BRAND.teal }} aria-hidden="true"></i>
          </div>
          <div style={{ marginBottom: 4 }}>
            <span style={{ fontWeight: 700, fontSize: 16, color: BRAND.navy }}>CIMSA </span>
            <span style={{ fontWeight: 700, fontSize: 16, color: BRAND.teal }}>SOLUCIONES</span>
          </div>
...
function WorkerView({ user, records, onRegister, onLogout }) {
  ...
  const myRecords = records.filter(r => r.userId === user.id).sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time));
  ...
  const handleRegister = (type) => {
    ...
    if (!navigator.geolocation) {
      setLoading(false);
      setStatusMsg("error:Tu dispositivo no soporta GPS. No es posible registrar.");
      return;
    }
    navigator.geolocation.getCurrentPosition(
      pos => {
        const { latitude: lat, longitude: lng } = pos.coords;
        onRegister({ userId: user.id, name: user.name, type, lat, lng, address: `${lat.toFixed(4)}, ${lng.toFixed(4)}` });
        setLoading(false);
        setStatusMsg(`ok:${type === "entrada" ? "Entrada" : "Salida"} registrada correctamente ✓`);
        setTimeout(() => setStatusMsg(""), 3000);
      },
      () => {
        setLoading(false);
        setStatusMsg("error:Ubicación bloqueada. Activa el GPS en tu navegador para poder registrarte.");
      },
      { timeout: 10000 }
    );
  };
    ...
    <div style={{ minHeight: "100vh", background: BRAND.navyLight }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "1.5rem 1rem" }}>
        <div style={{ background: "#fff", border: `0.5px solid #dde3f0`, borderRadius: 16, padding: "1.5rem", marginBottom: "1.5rem", textAlign: "center" }}>
          ...
          <p style={{ fontSize: 30, fontWeight: 500, margin: "0 0 1.5rem", color: BRAND.navy, fontVariantNumeric: "tabular-nums" }}>
            {new Date().toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}
          </p>

          {statusMsg && (() => {
            const isError = statusMsg.startsWith("error:");
            const msg = statusMsg.replace(/^(ok|error):/, "");
            return (
              <div style={{ background: isError ? "#fdecea" : BRAND.tealLight, color: isError ? "#c0392b" : BRAND.tealDark, borderRadius: 8, padding: "10px 14px", fontSize: 13, marginBottom: 16, display: "flex", alignItems: "flex-start", gap: 8, textAlign: "left" }}>
                <i className={`ti ${isError ? "ti-map-pin-off" : "ti-circle-check"}`} style={{ fontSize: 16, flexShrink: 0, marginTop: 1 }} aria-hidden="true"></i>
                <span>{msg}</span>
              </div>
            );
          })()}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <button onClick={() => handleRegister("entrada")} disabled={!canEntrada || loading}
              style={{ padding: "16px", fontSize: 15, background: canEntrada ? BRAND.tealLight : "#f5f5f5", color: canEntrada ? BRAND.tealDark : "#bbb", border: `1.5px solid ${canEntrada ? BRAND.teal : "#e0e0e0"}`, borderRadius: 10, cursor: canEntrada ? "pointer" : "not-allowed", transition: "opacity .15s" }}>
              <i className="ti ti-login" style={{ fontSize: 22, display: "block", marginBottom: 6 }} aria-hidden="true"></i>
              Entrada
            </button>
            <button onClick={() => handleRegister("salida")} disabled={!canSalida || loading}
              style={{ padding: "16px", fontSize: 15, background: canSalida ? BRAND.navyLight : "#f5f5f5", color: canSalida ? BRAND.navy : "#bbb", border: `1.5px solid ${canSalida ? BRAND.navy : "#e0e0e0"}`, borderRadius: 10, cursor: canSalida ? "pointer" : "not-allowed", transition: "opacity .15s" }}>
              <i className="ti ti-logout" style={{ fontSize: 22, display: "block", marginBottom: 6 }} aria-hidden="true"></i>
              Salida
            </button>
          </div>
          {loading && (
            <p style={{ fontSize: 12, color: "#888", marginTop: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <i className="ti ti-map-pin" style={{ fontSize: 14, color: BRAND.teal }} aria-hidden="true"></i>
              Obteniendo ubicación GPS...
            </p>
          )}
        </div>

        <h3 style={{ fontSize: 13, fontWeight: 500, color: "#888", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.04em" }}>Mis registros recientes</h3>
          ...
          <div key={r.id} style={{ background: "#fff", border: "0.5px solid #dde3f0", borderRadius: 10, padding: "10px 14px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: r.type === "entrada" ? BRAND.tealLight : BRAND.navyLight, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <i className={`ti ${r.type === "entrada" ? "ti-login" : "ti-logout"}`} style={{ fontSize: 15, color: r.type === "entrada" ? BRAND.teal : BRAND.navy }} aria-hidden="true"></i>
              </div>
              <div>
...
function AdminView({ records, onLogout, onAddUser, onToggleUser, allUsers }) {
  ...
  const exportCSV = () => {
    const header = "Nombre,Usuario,Tipo,Fecha,Hora,Latitud,Longitud,Dirección";
    const rows = filtered.map(r => `${r.name},${allUsers.find(u => u.id === r.userId)?.username || ""},${r.type},${r.date},${r.time},${r.lat},${r.lng},"${r.address}"`);
  ...
  const handleAddUser = () => {
    ...
    if (allUsers.find(u => u.username === newUser.username)) { setAddMsg("error:El usuario ya existe"); return; }
    onAddUser(newUser);
  ...
  const tabs = ["registros", "trabajadores"];
    ...
    <div style={{ minHeight: "100vh", background: BRAND.navyLight }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "1.5rem 1rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: "1.5rem" }}>
          {[
            { label: "Presentes ahora", value: presentToday, icon: "ti-user-check", color: BRAND.teal },
            { label: "Entradas hoy", value: entradas, icon: "ti-login", color: BRAND.navy },
            { label: "Total registros", value: records.length, icon: "ti-database", color: BRAND.tealDark },
          ].map(m => (
            <div key={m.label} style={{ background: "#fff", border: "0.5px solid #dde3f0", borderRadius: 12, padding: "1rem", display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: m.color === BRAND.teal ? BRAND.tealLight : BRAND.navyLight, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: "1.5rem", borderBottom: `2px solid #dde3f0`, paddingBottom: 0 }}>
          {tabs.map(t => (
          ...
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <button onClick={exportCSV} style={{ fontSize: 12, padding: "6px 14px", marginLeft: "auto", background: BRAND.navy, color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>
                <i className="ti ti-download" style={{ marginRight: 6 }} aria-hidden="true"></i>Exportar CSV
              </button>
            </div>
                ...
                <thead>
                  <tr style={{ background: BRAND.navy }}>
                    {["Nombre", "Tipo", "Fecha", "Hora", "Ubicación"].map(h => (
                      <th key={h} style={{ padding: "9px 12px", textAlign: "left", fontWeight: 500, fontSize: 12, color: "rgba(255,255,255,0.85)", borderBottom: "none" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.length === 0 && (
                    <tr><td colSpan={5} style={{ padding: "1.5rem", textAlign: "center", color: "#aaa", fontSize: 13 }}>Sin registros</td></tr>
                  )}
                  {filtered.map((r, i) => (
                    <tr key={r.id} style={{ borderBottom: i < filtered.length - 1 ? "0.5px solid #eef1f8" : "none", background: i % 2 === 0 ? "#fff" : "#fafbfd" }}>
                      <td style={{ padding: "8px 12px" }}>{r.name}</td>
                      <td style={{ padding: "8px 12px" }}>
                        <span style={{ fontSize: 11, padding: "2px 9px", borderRadius: 6, background: r.type === "entrada" ? BRAND.tealLight : BRAND.navyLight, color: r.type === "entrada" ? BRAND.tealDark : BRAND.navy, fontWeight: 500 }}>
                          {r.type}
                        </span>
                      </td>
                      <td style={{ padding: "8px 12px" }}>{r.date}</td>
                      <td style={{ padding: "8px 12px", fontVariantNumeric: "tabular-nums" }}>{r.time}</td>
          ...
          <>
            <div style={{ background: "#fff", border: "0.5px solid #dde3f0", borderRadius: 12, padding: "1.25rem", marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 8, alignItems: "end" }}>
                ...
                <button onClick={handleAddUser} style={{ fontSize: 13, padding: "8px 16px", background: BRAND.navy, color: "#fff", border: "none", borderRadius: 8, cursor: "pointer" }}>Agregar</button>
              </div>
              {addMsg && (() => {
...
const handleRegister = async ({ userId, name, type, lat, lng, address }) => {
  ...
  const handleToggleUser = (id) => {
    setAllUsers(prev => prev.map(u => u.id === id ? { ...u, active: u.active === false } : u));
  };
